...
public class SignUpAction {
	...
	public boolean signUpUser(HttpSession session, ServletContext context) throws SQLException, ClassNotFoundException
	{
		...
		Connection conn = DriverManager.getConnection(context.getInitParameter("url"), context.getInitParameter("username"), context.getInitParameter("password"));
		...
		if(conn != null)
		{
			try{
				String insert_user_details ="insert into user_details(name, gender, address, city, state, country) values ('"+
						session.getAttribute("name")+"','"+session.getAttribute("gender")+"','"+session.getAttribute("addr")
						+"','"+session.getAttribute("city")+"','"+session.getAttribute("state")+"','"+session.getAttribute("country")+"')";
